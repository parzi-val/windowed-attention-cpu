...
int main(void) {
    ...
    /* Dense C vs real HF logits -- ground truth. The matmul swap is the one thing most
     * likely to introduce a real bug here (row/col-major or transpose mixups are the
     * classic cblas_sgemm mistake) -- this and the gates below are exactly what would
     * catch that. */
    ...
    /* All 5 arms vs the dense reference, on the same short prompt (T < WINDOW=64), where
     * windowing at ANY fraction is mathematically required to reduce to dense exactly --
     * validates attention_forward_perhead's dense and windowed branches, AND that the
     * fraction-based masks (mask_from_fraction) select and thread through correctly. */
    int gates_ok = 1;
    for (int i = 0; i < 5; i++) {
